...
export default class MomentScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      username: UserInfoUtil.userInfo.username,
  ...
  getMoments(useLoading) {
    ...
    let url =
      Api.MOMENTS_URL + "?offset=" + this.offset + "&pagesize=" + this.pagesize;
    fetch(url)
      .then(res => res.json())
      .then(json => {
        if (useLoading) {
    ...
    return (
      <View>
        <Image
          ...
          source={require("../../images/moment.jpg")}
        />
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList
          ListHeaderComponent={() =>
            this.renderHeaderView(this.state.username, this.state.avatar)
          }
  ...
  renderReplys(item) {
    ...
    if (!Utils.isEmpty(arr) && arr.length > 0) {
      for (let i = 0; i < arr.length; i++) {
        replys.push(
          <Text
            key={item.item.moment_id + "-" + i}
            style={listItemStyle.commentText}
          >
  ...
  isCommentEmpty(item) {
    if (
      Utils.isEmpty(item.item.favor_names) &&
      (item.item.replys == null || item.item.replys.length == 0)
    ) {
      return true;
    }
    return false;
        ...
        <View style={listItemStyle.container}>
          ...
          <View style={listItemStyle.content}>
            ...
            <View style={listItemStyle.timeContainer}>
              ...
              >
                <Image
                  style={listItemStyle.commentImg}
                  ...
                  {Utils.isEmpty(item.item.favor_names) ? null : (
                    <View style={{ flexDirection: "column" }}>
                      <View style={listItemStyle.favorContainer}>
                        <Image
                          style={listItemStyle.favorImg}
                          source={require("../../images/ic_favor.png")}
                        />
                        <Text style={listItemStyle.commentText}>
                          {item.item.favor_names}
                        </Text>
                      </View>
                      {item.item.replys == null ||
                      item.item.replys.length == 0 ? null : (
                        <View
                          style={[
                            listItemStyle.divider,
  ...
  doCommentCallback = (momentId, momentUsername) => {
    this.refs.replyPopWin.showModal(
      momentId,
      momentUsername,
